...
from books.forms import NewBookForm,SearchForm
...
def userLogin(request):
    ...
    if request.method=="POST":
        ...
        if user:
            ...
            res=render(request,'books/user_login.html',data)
            return res
    else:
...
def viewBooks(request):
    ...
    res=render(request,'books/view_book.html',{'books':books,'username':username})
    return res
@login_required(login_url="/books/login/")
def newBook(request):
    form=NewBookForm()
